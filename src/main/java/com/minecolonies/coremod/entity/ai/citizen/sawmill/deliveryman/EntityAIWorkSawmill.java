...
public class EntityAIWorkSawmill extends AbstractEntityAIInteract<JobSawmill>
{
    ...
    public AIState collectItems()
    {
        ...
        if (!needsCurrently.isEmpty())
        {
            final Predicate<ItemStack> targetPredicate = needsCurrently.stream().filter(stackPredicate -> !InventoryUtils.hasItemInProvider(worker.getCitizenData(), stackPredicate)).findFirst().orElse(null);
